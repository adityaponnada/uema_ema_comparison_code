...
construct_plot <- function(df, construct) {
  ...
  plot <- ggplot(plot_df, aes(x = plot_df[,1], y = plot_df[,2])) +
    ...
    labs(title = paste("Scatterplot for", construct),
         x = paste("uema_", construct),
         y = paste("ema_", construct))
  return(plot)
}
...
construct_plots_grid <- plot_grid(plotlist = construct_plots, ncol = 4, nrow = 3)
